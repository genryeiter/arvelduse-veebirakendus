...
const Invoice = () => {
    ...
    useEffect(() => {
        dispatch(getClientsByUser({search: user?.result?._id || user?.result?.sub}));
        // eslint-disable-next-line
    }, [dispatch]);
    ...
    const defaultProps = {
        options: currencies, getOptionLabel: (option) => option.label
    };

    const clientsProps = {
        options: clients, getOptionLabel: (option) => option.name
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (invoice) {
            ...
            dispatch(createInvoice({
                ...
                currency: currency,
                dueDate: selectedDate,
                invoiceNumber: `${invoiceData.invoiceNumber < 100 ? (Number(invoiceData.invoiceNumber)).toString().padStart(3, '0') : Number(invoiceData.invoiceNumber)}`,
                client,
                type: type,
                status: status,
                paymentRecords: [],
    ...
    return (<div className={styles.invoiceLayout}>
            ...
            <Container className={classes.headerContainer}>
                <Grid container justifyContent="flex-end">
                    <Grid item>
                        ...
                        >
                                <span style={{
                                    width: '40px', color: 'black', padding: '15px',
                                }}
                        ...
                        <TableBody>
                            {invoiceData.items.map((itemField, index) => (<TableRow key={index}>
                                <TableCell scope="row" style={{width: '40%'}}> <InputBase
                                    ...
                                    value={itemField.itemName} placeholder="Item name or description"/>
                                </TableCell>
